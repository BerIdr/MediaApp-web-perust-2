...
const Likes = ({item}: {item: MediaItemWithOwner}) => {
  ...
  return (
    <>
      <p className="text-center text-lg font-bold mb-2 text-gray-800">Likes: {likeState.count}</p>
      <button
        className="block w-full cursor-pointer rounded-md bg-blue-600 p-2 text-center text-white font-semibold transition-all duration-500 ease-in-out hover:bg-blue-800"
        onClick={handleLike}
      >
